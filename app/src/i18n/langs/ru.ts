const ru = {
  message: {
    ...
    close_to_edit_connection:
      'Вы должны закрыть соединение перед редактированием. Вы уверены, что хотите продолжить?',
    ...
    enter_to_exec:
      'После ввода команды Redis нажмите клавишу Enter для выполнения, клавиши вверх и вниз для переключения истории команд',
    pre_version: 'Версия',
    ...
    info_disabled:
      'Исключение при выполнении команды Info (возможно, было отключено), информация о Redis не может быть отображена.',
    page_zoom: 'Масштаб страницы',
    scan_disabled:
      'Исключение при выполнении команды Scan (возможно, было отключено), невозможно отобразить список ключей.',
    ...
    key_no_permission:
      'Срок действия разрешения на чтение файла истек, пожалуйста, повторно выберите ключевой файл вручную',
    toggle_check_all: 'выбрать все | отменить выбор',
    select_lang: 'Выбрать язык',
    clear_cache_tip:
      'Когда возникает проблема с клиентом, эта операция удалит все подключения и конфигурации для восстановления клиента',
    detail: 'Деталь',
    separator_tip:
      'Разделитель древовидного представления, установлен на пустой, чтобы отключить дерево и отображать ключи в виде списка',
